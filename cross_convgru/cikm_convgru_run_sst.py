...
parser.add_argument('--save_dir', type=str, default='checkpoints/cikm_convgru_SST')
parser.add_argument('--gen_frm_dir', type=str, default='result/CIKM_convgru_SST/')
...
def wrapper_valid(model):
    ...
    while flag:
        ...
        dat, (index, b_cup) = sample5(args.batch_size, data_type='validation', index=index)
        # dat = nor(dat)
        tars = dat[:, -output_length:]
        # ims = padding_CIKM_data(dat)
        ims=dat
...
def wrapper_train(model):
    ...
    for itr in (range(1, args.max_iterations + 1)):
        ims = sample5(batch_size=batch_size
        )
        # print(ims.shape)
