...
parser.add_argument('--save_dir', type=str, default='checkpoints/cikm_convgru_POC')
parser.add_argument('--gen_frm_dir', type=str, default='result/CIKM_convgru_POC/')
...
def wrapper_valid(model):
    ...
    while flag:
        ...
        dat, (index, b_cup) = sample4(args.batch_size, data_type='validation', index=index)
        # dat = nor(dat)
        tars = dat[:, -output_length:]
        # ims = padding_CIKM_data(dat)
        ims=dat
...
def wrapper_train(model):
    ...
    for itr in (range(1, args.max_iterations + 1)):
        ims = sample4(batch_size=batch_size
        )
        # print(ims.shape)
